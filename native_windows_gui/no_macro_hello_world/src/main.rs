...
mod basic_app_ui {
    ...
    impl nwg::NativeUi<BasicAppUi> for BasicApp {
        fn build_ui(mut data: Self) -> Result<BasicAppUi, nwg::NwgError> {
            ...
            let handler_events = move | evt, _evt_data, handle | {
                if let Some(ui) = evt_ui.upgrade() {
                    match evt {
                        E::OnButtonClick => if &handle == &ui.hello_button {
            ...
            *ui.default_handler.borrow_mut() = Some(nwg::full_bind_event_handler(&ui.window.handle, handler_events));
